...
// ============ AUTH ============
function canManageMachines(){ return currentUser && ['Maintenance Supervisor','Management'].includes(currentUser.role); }
function isManagement(){ return currentUser && currentUser.role === 'Management'; }
...
async function showAppShell(){
  ...
  document.getElementById('headerSubtitle').textContent = orgName
...
document.getElementById('loginForm').addEventListener('submit', async (e)=>{
  ...
  try{
    currentUser = await apiPost('/auth/login', {
      orgCode: document.getElementById('loginOrgCode').value.trim(),
...
document.getElementById('logForm').addEventListener('submit', async (e)=>{
  ...
  submitBtn.disabled = true;
  try{
    const created = await apiPost('/logs', {
      machineId,
    ...
    switchView('logs');
  }catch(err){ showToast('Could not submit log: '+err.message); }
  finally{ submitBtn.disabled = false; }
});
  ...
  try{
    ...
    list.innerHTML = users.map(u=>`
      <div class="user-row">
        ...
        <span class="role-pill ${u.role==='Management'?'mgmt':''}">${u.role}</span>
...
document.getElementById('orgBrandingForm').addEventListener('submit', async (e)=>{
  ...
  try{
    ...
    const orgName = document.getElementById('brandName').value.trim();
    document.getElementById('headerSubtitle').textContent = `${orgName} \u2014 MAINTENANCE LOG & WORK ORDER SYSTEM`;
...
async function pdfHeader(doc, title, subtitle){
  ...
  if(logo){
    try{
      ...
      doc.addImage(logo, fmt, 14, 6, 18, 18);
      textX = 36;
    }catch(e){ textX = 14; }
  }
  ...
  doc.setFontSize(15);
  doc.text(b.companyName || 'ZENITH STEEL FABRICATORS LTD', textX, 15);
  doc.setFont(undefined,'normal');
  doc.setFontSize(8);
  doc.setTextColor(107,114,128);
  doc.text(b.tagline || 'DESIGN, FABRICATE & ERECT STRUCTURAL STEEL WORK', textX, 20);
