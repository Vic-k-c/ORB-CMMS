...
// Re-verifies the current user's own password from the request body -
// used as an extra confirmation step before genuinely destructive actions
// (deleting a log or machine), on top of the normal permission check.
// Confirms it's really them at the keyboard, not just that their session
// happens to still be logged in.
function requirePasswordConfirmation() {
  return async (req, res, next) => {
    try {
      const { password } = req.body || {};
      if (!password) return res.status(400).json({ error: 'Password required to confirm this action' });
      const user = await dbGet('SELECT password_hash FROM users WHERE id = $1', [req.session.user.id]);
      if (!user || !bcrypt.compareSync(password, user.password_hash)) {
        return res.status(401).json({ error: 'Incorrect password' });
      }
      next();
    } catch (e) { res.status(500).json({ error: 'Could not verify password' }); }
  };
}
...
app.delete('/api/machines/:id', requirePermission('manage_machines'), requirePasswordConfirmation(), async (req, res) => {
  try {
    const existing = await dbGet('SELECT id, name FROM machines WHERE id = $1 AND organization_id = $2', [req.params.id, req.session.user.organizationId]);
    if (!existing) return res.status(404).json({ error: 'Machine not found' });
    const logCount = await dbGet('SELECT COUNT(*) as c FROM logs WHERE machine_id = $1', [req.params.id]);
    if (toInt(logCount.c) > 0) {
      return res.status(409).json({ error: `"${existing.name}" has ${logCount.c} maintenance log(s) attached - delete those first if you're sure, rather than losing that history silently.` });
    }
    await dbRun('DELETE FROM machines WHERE id = $1', [req.params.id]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: 'Could not delete machine: ' + e.message }); }
});
...
app.delete('/api/logs/:id', requirePermission('manage_logs'), requirePasswordConfirmation(), async (req, res) => {
  try {
    const log = await dbGet('SELECT id FROM logs WHERE id = $1 AND organization_id = $2', [req.params.id, req.session.user.organizationId]);
    if (!log) return res.status(404).json({ error: 'Log not found' });
...
app.get('/api/dashboard', async (req, res) => {
  try {
    ...
    const machinesDownList = (await dbAll(
      "SELECT * FROM machines WHERE organization_id = $1 AND status = 'Down' ORDER BY name ASC", [orgId]
    )).map(toMachineJSON);
