...
// Compares one day's breakdown count, total logs, and downtime against the
// same calendar date a day, a month, and a year earlier - giving simple,
// easy-to-read deltas ("up from yesterday") without cluttering the graph
// itself. Note: for month/year comparisons this uses JS Date's own
// day-rollover behavior for dates that don't exist in the earlier month
// (e.g. there's no Feb 31, so Mar 31 - 1 month rolls to Mar 3) - a known,
// rare edge case rather than something requiring full calendar-safe logic
// for one day a year.
async function computeDailyComparison(organizationId, dateStr) {
  ...
  const lastMonth = new Date(d); lastMonth.setUTCMonth(d.getUTCMonth() - 1);
  const lastYear = new Date(d); lastYear.setUTCFullYear(d.getUTCFullYear() - 1);
  ...
  const [current, y, lm, ly] = await Promise.all([
    countsForDate(dateStr), countsForDate(toDateStr(yesterday)), countsForDate(toDateStr(lastMonth)), countsForDate(toDateStr(lastYear))
  ]);

  function delta(curVal, refVal) {
  ...
  return {
    breakdownCount: { current: current.breakdownCount, vsYesterday: delta(current.breakdownCount, y.breakdownCount), vsLastMonth: delta(current.breakdownCount, lm.breakdownCount), vsLastYear: delta(current.breakdownCount, ly.breakdownCount) },
    totalLogs: { current: current.totalLogs, vsYesterday: delta(current.totalLogs, y.totalLogs), vsLastMonth: delta(current.totalLogs, lm.totalLogs), vsLastYear: delta(current.totalLogs, ly.totalLogs) },
    totalDowntime: { current: current.totalDowntime, vsYesterday: delta(current.totalDowntime, y.totalDowntime), vsLastMonth: delta(current.totalDowntime, lm.totalDowntime), vsLastYear: delta(current.totalDowntime, ly.totalDowntime) }
  };
}
...
app.get('/api/reports/monthly', async (req, res) => {
  try {
    ...
    res.json({ year, month, dailyBreakdown, totalLogs, totalDowntime, preventiveCount, breakdownCount, partsUsedSummary });
...
app.get('/api/reports/yearly', async (req, res) => {
  try {
    ...
    res.json({ year, monthlyBreakdown, totalLogs, totalDowntime, preventiveCount, breakdownCount, partsUsedSummary });
