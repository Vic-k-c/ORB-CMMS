...
const ROLES = ['Technician', 'Maintenance Supervisor', 'Management'];
...
// Separate storage for organization logos. Restricted to PNG/JPEG only (not
// arbitrary image types) because both jsPDF (client-side PDF generation) and
// ExcelJS (server-side Excel generation) only reliably embed those two formats.
...
function logoExtensionFor(mimeType) {
...
function addLetterheadLogo(workbook, sheet, org) {
  ...
  const filePath = path.join(logosDir, org.logo_url);
...
app.put('/api/organization/branding', requireRole('Management'), async (req, res) => {
  try {
    ...
    const {
      ...
      docNoExcel, docEffectiveDateExcel, docRevExcel, approvedBy
    } = req.body;
    const updated = {
      name: name !== undefined ? name.trim() : existing.name,
    ...
    await dbRun(`
      UPDATE organizations SET name=$1, tagline=$2, doc_no_log=$3, doc_effective_date_log=$4, doc_rev_log=$5,
        doc_issue_log=$6, doc_no_excel=$7, doc_effective_date_excel=$8, doc_rev_excel=$9, approved_by=$10
      WHERE id=$11
    `, [updated.name, updated.tagline, updated.doc_no_log, updated.doc_effective_date_log, updated.doc_rev_log,
        updated.doc_issue_log, updated.doc_no_excel, updated.doc_effective_date_excel, updated.doc_rev_excel, updated.approved_by, existing.id]);
    res.json(toOrgJSON(await dbGet('SELECT * FROM organizations WHERE id = $1', [existing.id])));
...
app.get('/api/organization/logo', async (req, res) => {
  try {
    const org = await dbGet('SELECT logo_url, logo_mime_type FROM organizations WHERE id = $1', [req.session.user.organizationId]);
    if (!org || !org.logo_url) return res.status(404).json({ error: 'No logo uploaded' });
    const filePath = path.join(logosDir, org.logo_url);
...
app.post('/api/organization/logo', requireRole('Management'), (req, res) => {
  logoUpload.single('logo')(req, res, async (err) => {
    if (err) return res.status(400).json({ error: err.message });
    if (!req.file) return res.status(400).json({ error: 'No logo file received' });
    try {
      const orgId = req.session.user.organizationId;
      const existing = await dbGet('SELECT logo_url FROM organizations WHERE id = $1', [orgId]);
      if (existing && existing.logo_url) {
        const oldPath = path.join(logosDir, existing.logo_url);
        if (fs.existsSync(oldPath)) fs.unlinkSync(oldPath);
      }
      const filename = `${orgId}.${logoExtensionFor(req.file.mimetype)}`;
      fs.writeFileSync(path.join(logosDir, filename), req.file.buffer);
      await dbRun('UPDATE organizations SET logo_url = $1, logo_mime_type = $2 WHERE id = $3', [filename, req.file.mimetype, orgId]);
      res.status(201).json({ ok: true });
    } catch (e) { res.status(500).json({ error: 'Could not save logo: ' + e.message }); }
  });
});

app.delete('/api/organization/logo', requireRole('Management'), async (req, res) => {
  try {
    const orgId = req.session.user.organizationId;
    const existing = await dbGet('SELECT logo_url FROM organizations WHERE id = $1', [orgId]);
    if (existing && existing.logo_url) {
    ...
    await dbRun('UPDATE organizations SET logo_url = NULL, logo_mime_type = NULL WHERE id = $1', [orgId]);
    res.json({ ok: true });
...
app.get('/api/users', requireRole('Management'), async (req, res) => {
  try {
...
app.post('/api/users', requireRole('Management'), async (req, res) => {
  try {
...
app.put('/api/users/:id', requireRole('Management'), async (req, res) => {
  try {
    ...
    if (role && role !== 'Management' && existing.role === 'Management') {
      const mgmtCount = await dbGet("SELECT COUNT(*) as c FROM users WHERE role = 'Management' AND organization_id = $1", [req.session.user.organizationId]);
      if (toInt(mgmtCount.c) <= 1) return res.status(400).json({ error: 'Cannot demote the last Management account' });
    }
    const updated = {
      name: name !== undefined ? name.trim() : existing.name,
...
app.delete('/api/users/:id', requireRole('Management'), async (req, res) => {
  try {
    const existing = await dbGet('SELECT * FROM users WHERE id = $1 AND organization_id = $2', [req.params.id, req.session.user.organizationId]);
    if (!existing) return res.status(404).json({ error: 'User not found' });
    if (existing.role === 'Management') {
      const mgmtCount = await dbGet("SELECT COUNT(*) as c FROM users WHERE role = 'Management' AND organization_id = $1", [req.session.user.organizationId]);
      if (toInt(mgmtCount.c) <= 1) return res.status(400).json({ error: 'Cannot delete the last Management account' });
    }
...
app.get('/api/machines/metrics-matrix', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
    const orgId = req.session.user.organizationId;
    const { fromDate, toDate } = resolvePeriod(req.query);
    const machines = await dbAll('SELECT * FROM machines WHERE organization_id = $1 ORDER BY name ASC', [orgId]);
    const rows = [];
...
app.get('/api/machines/metrics-matrix/export/excel', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
    const orgId = req.session.user.organizationId;
    ...
    periodCell.value = `PERIOD: ${fmtDateForExcel(fromDate)} to ${fmtDateForExcel(toDate)}`;
...
app.post('/api/machines', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
...
app.put('/api/machines/:id', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
...
app.get('/api/settings', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
...
app.put('/api/settings', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
...
app.get('/api/schedule-overrides', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
...
app.post('/api/schedule-overrides', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
...
app.delete('/api/schedule-overrides/:date', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
...
app.post('/api/logs', async (req, res) => {
  try {
    const {
      ...
      downtimeHours, findings, actionsTaken, partsUsed, status
    } = req.body;
    ...
    const log = {
      ...
      parts_used: (partsUsed || '').trim(), status, logged_at: nowISO()
    };
...
app.put('/api/logs/:id', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
    const orgId = req.session.user.organizationId;
    const existing = await dbGet('SELECT * FROM logs WHERE id = $1 AND organization_id = $2', [req.params.id, orgId]);
    if (!existing) return res.status(404).json({ error: 'Log not found' });

    const { machineId, logType, reportedBy, priority, technician, downtimeHours, findings, actionsTaken, partsUsed, status } = req.body;
    ...
    await dbRun(`
      UPDATE logs SET machine_id=$1, log_type=$2, reported_by=$3, priority=$4, technician=$5,
        downtime_hours=$6, findings=$7, actions_taken=$8, parts_used=$9, status=$10
      WHERE id=$11
    `, [updated.machine_id, updated.log_type, updated.reported_by, updated.priority, updated.technician,
        updated.downtime_hours, updated.findings, updated.actions_taken, updated.parts_used, updated.status, req.params.id]);
...
app.patch('/api/logs/:id/review', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
    const orgId = req.session.user.organizationId;
...
app.delete('/api/attachments/:id', requireRole('Maintenance Supervisor', 'Management'), async (req, res) => {
  try {
    const a = await dbGet(`
      SELECT att.* FROM attachments att JOIN logs l ON l.id = att.log_id
      WHERE att.id = $1 AND l.organization_id = $2
...
function toOrgJSON(row) {
  return {
    ...
    logoUrl: row.logo_url ? '/api/organization/logo' : null,
