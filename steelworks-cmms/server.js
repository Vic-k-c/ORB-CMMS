...
app.get('/api/reports/monthly', async (req, res) => {
  try {
    ...
    const partsRows = await dbAll(`
      SELECT l.logged_at, l.parts_used, m.name as machine_name, m.code as machine_code
      FROM logs l JOIN machines m ON m.id = l.machine_id
    ...
    const partsUsedSummary = partsRows.map(r => ({
      date: r.logged_at.slice(0, 10), machineName: r.machine_name, machineCode: r.machine_code, partsUsed: r.parts_used
    }));
