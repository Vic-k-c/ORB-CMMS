...
document.getElementById('machineForm').addEventListener('submit', async (e)=>{
  ...
  try{
    ...
    if(fileInput.files.length > 0){
      ...
      try{
        const resp = await fetch(API+'/machines/'+machineId+'/photo', { method:'POST', credentials:'same-origin', body: fd });
        if(!resp.ok){ const err = await resp.json().catch(()=>({})); throw new Error(err.error || 'Photo upload failed'); }
...
function logEntryHtml(l, showMachine){
  ...
  return `
    <div class="log-entry ${l.logType.toLowerCase()}">
      ...
      ${attachments.length>0 ? `<div class="attachment-list">${attachments.map(a=>`<span class="attachment-chip"><a href="${a.url}" target="_blank" rel="noopener" title="Uploaded by ${escapeHtml(a.uploadedBy||'unknown')} on ${fmtDateTime(a.uploadedAt)}">&#128206; ${escapeHtml(a.originalName)}</a>${canManageLogs() ? `<button class="del" onclick="deleteAttachment('${a.id}','${l.id}')" title="Delete attachment">&times;</button>` : ''}</span>`).join('')}</div>` : ''}
