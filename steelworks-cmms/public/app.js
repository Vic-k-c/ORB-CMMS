...
function renderCategoryTabs(){
  ...
  wrap.innerHTML = tabs.map(c=>{
    ...
    const active = machineState.category===c ? 'active' : '';
    return `<button class="category-tab ${active}" onclick="setMachineCategory('${escapeHtml(c).replace(/'/g,"\\'")}')">${escapeHtml(c)}<span class="tab-count">${count}</span></button>`;
  }).join('');
}
