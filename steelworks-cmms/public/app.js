...
function openAttachmentViewer(url, name, mimeType){
  ...
  const content = isImage
    ? `<img src="${url}" alt="${escapeHtml(name)}" style="max-width:100%;max-height:75vh;display:block;margin:0 auto;border-radius:4px;">`
    : isPdf
    ? `<iframe src="${url}" style="width:100%;height:75vh;border:0;border-radius:4px;"></iframe>`
  ...
  modal.innerHTML = `
    <div class="attachment-viewer-box">
      ...
      <div class="attachment-viewer-body">${content}</div>
    </div>`;
  ...
  return `
    <div class="log-entry ${l.logType.toLowerCase()}">
      ...
      <div class="log-entry-actions">
        ...
        ${canManageLogs() ? `<button class="log-delete-btn" onclick="deleteLog('${l.id}','${escapeHtml(l.machineCode||'this machine')}')">&#128465; Delete</button>` : ''}
...
async function deleteLog(id, machineCode){
  if(!confirm(`Delete this log for ${machineCode}? This removes it and any attached files permanently - this cannot be undone.`)) return;
  try{
    await apiDelete('/logs/'+id);
    showToast('Log deleted');
    refreshActiveView();
  }catch(e){ showToast('Could not delete log: '+e.message); }
}
    ...
    <tr>
      ...
      <td class="nowrap">
        <div class="icon-btn-row">
          ...
          ${canManageLogs() ? `<button class="icon-btn" title="Delete" onclick="deleteLog('${l.id}','${escapeHtml(l.machineCode||'this machine')}')">&#128465;</button>` : ''}
        </div>
      </td>
    </tr>`;
}
