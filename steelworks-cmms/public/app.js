...
// Reusable password re-confirmation prompt for destructive actions
// (deleting a log or machine). onConfirm receives the entered password
// and should throw with a message on failure - the modal shows that
// message and stays open rather than closing on a failed attempt.
function confirmWithPassword(title, message, onConfirm){
  const modal = document.createElement('div');
  modal.className = 'attachment-viewer-overlay';
  modal.innerHTML = `
    <div class="password-confirm-box">
      <div class="password-confirm-title">${escapeHtml(title)}</div>
      <div class="password-confirm-body">${escapeHtml(message)}</div>
      <input type="password" id="pwConfirmInput" placeholder="Enter your password to confirm" autocomplete="current-password">
      <div class="password-confirm-error" id="pwConfirmError" style="display:none;"></div>
      <div class="password-confirm-actions">
        <button class="btn-ghost" id="pwConfirmCancel">Cancel</button>
        <button class="log-delete-btn" id="pwConfirmOk">Delete</button>
      </div>
    </div>`;
  document.body.appendChild(modal);
  const input = modal.querySelector('#pwConfirmInput');
  const errEl = modal.querySelector('#pwConfirmError');
  const confirmBtn = modal.querySelector('#pwConfirmOk');
  input.focus();
  modal.querySelector('#pwConfirmCancel').onclick = () => modal.remove();
  modal.onclick = (e) => { if(e.target === modal) modal.remove(); };
  const doConfirm = async () => {
    if(!input.value){ errEl.textContent = 'Enter your password to confirm'; errEl.style.display='block'; return; }
...
window.confirmWithPassword = confirmWithPassword;

function openAttachmentViewer(url, name, mimeType){
  const isImage = mimeType && mimeType.startsWith('image/');
  const isPdf = mimeType === 'application/pdf';
  // PDFs are documents meant to be read, not glanced at - they need real
  // screen space, especially on a PC monitor. A modal sized for a quick
  // photo look (a few hundred px) makes a PDF page feel cramped and tiny.
  // Images stay at a moderate size instead - the browser's native image
  // view (click-to-zoom, right-click-to-save) already handles a closer
  // look without needing the whole screen for it.
  const content = isImage
    ? `<img src="${url}" alt="${escapeHtml(name)}" style="max-width:100%;max-height:78vh;display:block;margin:0 auto;border-radius:4px;">`
    : isPdf
    ? `<iframe src="${url}" style="width:100%;height:100%;border:0;"></iframe>`
    : `<div class="empty-state"><div class="big">Preview not available</div><div>This file type can't be previewed in-app - use Download instead.</div></div>`;
  const modal = document.createElement('div');
  modal.className = 'attachment-viewer-overlay';
  modal.innerHTML = `
    <div class="attachment-viewer-box ${isPdf ? 'pdf-size' : ''}">
      <div class="attachment-viewer-head">
        <span>${escapeHtml(name)}</span>
        <div style="display:flex;gap:8px;align-items:center;">
          <a href="${url}" download="${escapeHtml(name)}" class="btn-ghost small">&#11015; Download</a>
          <button class="btn-ghost small" onclick="this.closest('.attachment-viewer-overlay').remove()">&times; Close</button>
        </div>
      </div>
      <div class="attachment-viewer-body ${isPdf ? 'pdf-body' : ''}">${content}</div>
    </div>`;
  modal.onclick = (e) => { if(e.target === modal) modal.remove(); };
  document.body.appendChild(modal);
}
...
// Splits a stored UTC ISO timestamp into separate local date/time strings,
// for populating the Start/End date and time inputs when editing a log.
function isoToLocalDateTimeParts(iso){
  const d = new Date(iso);
  const date = d.getFullYear() + '-' + String(d.getMonth()+1).padStart(2,'0') + '-' + String(d.getDate()).padStart(2,'0');
  const time = String(d.getHours()).padStart(2,'0') + ':' + String(d.getMinutes()).padStart(2,'0');
  return { date, time };
}
// Combines a date input's "YYYY-MM-DD" with a time input's "HH:MM" (both
// plain local values with no timezone info) into a UTC ISO string - the
// Date constructor interprets these components as local time, and
// toISOString() converts correctly from there. Returns null if either
// piece is missing (e.g. the form hasn't been filled in yet).
...
function nowLocalDateStr(){ return toLocalDateInputValue(new Date().toISOString()); }
function nowLocalTimeStr(){
  const d = new Date();
  return String(d.getHours()).padStart(2,'0') + ':' + String(d.getMinutes()).padStart(2,'0');
}
// Live-recalculates the read-only Downtime field as Start/End change, and
// surfaces a clear error if End isn't after Start - this is the only place
// downtime is computed on the client; the server recomputes it again
// authoritatively rather than trusting whatever the client sends.
// End Date is optional in the form (auto-follows Start Date via the
// listener below) - this is the single place that resolves "what End
// Date actually applies", falling back to Start Date if somehow empty,
// so every other function doesn't need to duplicate that fallback logic.
function effectiveEndDate(){
  const endVal = document.getElementById('logEndDate').value;
  return endVal || document.getElementById('logStartDate').value;
}

function updateLogDowntimeDisplay(){
  const startISO = localDateTimeToISO(document.getElementById('logStartDate').value, document.getElementById('logStartTime').value);
  const endISO = localDateTimeToISO(effectiveEndDate(), document.getElementById('logEndTime').value);
  const downtimeField = document.getElementById('logDowntime');
  const errEl = document.getElementById('logTimeError');
  if(!startISO || !endISO){ downtimeField.value = ''; errEl.style.display = 'none'; return; }
  ...
  downtimeField.value = hours.toFixed(2) + ' hour' + (Math.abs(hours - 1) < 0.001 ? '' : 's');
}
...
function showToast(msg, persistent){
  ...
  if(!persistent) setTimeout(()=>t.remove(),4500);
}

// ============ IDLE SESSION TIMEOUT ============
// 15 minutes of inactivity triggers a warning with a 60-second countdown;
// if there's no response, the user is signed out automatically. Chosen as
// a reasonable default for a shared/shop-floor device - adjust
// IDLE_TIMEOUT_MS if that's too short or too long in practice.
const IDLE_TIMEOUT_MS = 15 * 60 * 1000;
const IDLE_WARNING_MS = 60 * 1000;
let idleWarnTimer = null, idleLogoutTimer = null, idleCountdownInterval = null, idleTrackingStarted = false;
...
function resetIdleTimer(){
  ...
  const existingWarning = document.getElementById('idleWarningOverlay');
...
function showIdleWarning(){
  ...
  overlay.innerHTML = `
    <div class="idle-warning-box">
      <div class="idle-warning-title">Still there?</div>
      <div class="idle-warning-body">You've been inactive for a while. For security, you'll be signed out in <span id="idleCountdownNum">${secondsLeft}</span> seconds.</div>
      <button class="btn-primary" onclick="staySignedIn()">Stay Signed In</button>
    </div>`;
...
function startIdleTracking(){
  if(idleTrackingStarted) { resetIdleTimer(); return; }
  idleTrackingStarted = true;
  let throttled = false;
  ['mousedown','keydown','touchstart','scroll'].forEach(evt => document.addEventListener(evt, ()=>{
    if(throttled || !currentUser) return;
    throttled = true;
...
// ============ AUTH ============
// Checks the resolved permissions array from login/session (works
// uniformly for both built-in and custom roles - the server computes
// a built-in role's implicit permissions the same way either way).
function hasPermission(perm){ return currentUser && Array.isArray(currentUser.permissions) && currentUser.permissions.includes(perm); }
function canManageMachines(){ return hasPermission('manage_machines'); }
function canManageLogs(){ return hasPermission('manage_logs'); }
function canSubmitLogs(){ return hasPermission('submit_logs'); }
function canViewReports(){ return hasPermission('view_reports'); }
// isManagement() stays role-name-based, not permission-based: user
// management and organization branding are deliberately kept built-in-role
// only (Management/Maintenance HOD), not extendable via custom roles even
// though "manage_users"/"manage_branding" exist as selectable permissions -
// extending those specifically would let a custom role create new admin
// accounts, which is a real privilege-escalation risk beyond what custom
// roles are meant to cover here.
...
async function showAppShell(){
  ...
  if(isSuperAdmin()){
    ...
    document.getElementById('superAdminUserChip').textContent = currentUser.name;
    loadOrganizations();
    return;
  }
  ...
  document.getElementById('mobileUserInfo').textContent = currentUser.name;
  document.getElementById('adminNavBtn').style.display = isManagement() ? '' : 'none';
  document.getElementById('machinesNavBtn').style.display = hasPermission('view_machines') ? '' : 'none';
  document.getElementById('logsNavBtn').style.display = (hasPermission('submit_logs') || hasPermission('manage_logs')) ? '' : 'none';
  document.getElementById('reportsNavBtn').style.display = hasPermission('view_reports') ? '' : 'none';
  document.getElementById('dashboardNewLogBtn').style.display = canSubmitLogs() ? '' : 'none';
  document.getElementById('logsNewLogBtn').style.display = canSubmitLogs() ? '' : 'none';
  await loadBranding();
  applyHeaderBranding();
  loadDashboard();
}
...
document.getElementById('loginOrgCode').addEventListener('input', (e)=>{
  e.target.classList.remove('input-error');
});
document.getElementById('loginForm').addEventListener('submit', async (e)=>{
  e.preventDefault();
  const errEl = document.getElementById('loginError');
  const orgCodeEl = document.getElementById('loginOrgCode');
  errEl.style.display = 'none';
  orgCodeEl.classList.remove('input-error');
  const orgCodeTyped = orgCodeEl.value.trim();
  const submitBtn = e.target.querySelector('button[type="submit"]');
  submitBtn.disabled = true;
  try{
    // Deliberately NOT using apiPost here - its shared error handling
    // treats every 401 as an expired session and overwrites the message
    // with "Session expired", which is actively misleading for a login
    // attempt that simply failed (e.g. forgot the company code). A plain
    // fetch preserves whatever specific message the server actually sent.
    ...
    if(!res.ok) throw new Error(data.error || 'Login failed - please check your details and try again');
    currentUser = data;
    if(orgCodeTyped) localStorage.setItem('lastOrgCode', orgCodeTyped);
    document.getElementById('loginForm').reset();
    showAppShell();
  }catch(err){
    // The empty-code case gets a visual highlight instead of a text
    // message, since the field being blank is self-explanatory once
    // it's visibly flagged - a paragraph of text explaining it isn't
    // needed. Every other failure (wrong password, wrong code value,
    // etc.) still gets the actual message, since a highlighted field
    // alone wouldn't tell the user what's actually wrong in those cases.
    if(!orgCodeTyped){
      orgCodeEl.classList.add('input-error');
      orgCodeEl.focus();
    } else {
...
// Shows the back-to-top button once the page has scrolled past one
// screen's worth of content, hides it near the top.
...
// ============ SHARED: findings rendering (Dashboard + Machine detail) ============
// panelId is the wrapper to show/hide (hidden entirely when there are no
// findings, rather than showing an empty "Automatic Findings" panel with
// nothing in it). Cards for a specific machine are clickable through to
// that machine's detail page; whole-plant/category findings are not.
function renderFindings(findings, panelId, listId){
  const panel = document.getElementById(panelId);
  const list = document.getElementById(listId);
  if(!findings || findings.length===0){ panel.style.display='none'; return; }
  panel.style.display='';
  list.innerHTML = findings.map(f=>{
    const clickable = f.machineId ? 'clickable' : '';
    const onclick = f.machineId ? `onclick="openMachineDetail('${f.machineId}')"` : '';
    return `
    <div class="finding-card ${f.severity} ${clickable}" ${onclick}>
      <div class="finding-card-head">
        <span class="finding-severity ${f.severity}">${f.severity}</span>
        <span class="finding-title">${escapeHtml(f.title)}</span>
      </div>
      <div class="finding-detail">${escapeHtml(f.detail)}</div>
      <div class="finding-action">Suggested: ${escapeHtml(f.suggestedAction)}</div>
    </div>`;
  }).join('');
}
...
async function loadDashboard(){
  try{
    ...
    window._machinesDownListData = data.machinesDownList || [];
    renderMachinesDownList();
    ...
    renderFindings(data.findings, 'findingsPanel', 'findingsList');
        ...
        <div class="attention-card" onclick="openMachineDetail('${m.id}')">
          <div class="attention-top">
            <div>
              ...
              <div class="attention-meta">Category: ${escapeHtml(m.department||'\u2014')}</div>
            </div>
...
function renderMachinesDownList(){
  const list = window._machinesDownListData || [];
  const el = document.getElementById('machinesDownList');
  if(list.length === 0){
    el.innerHTML = '<div class="empty-state"><div class="big">No machines are down right now</div></div>';
    return;
  }
  el.innerHTML = list.map(m => `
    <div class="recent-log-row clickable" onclick="openMachineDetail('${m.id}')">
      <div>
        <b>${escapeHtml(m.name)}</b>
        <div style="color:var(--grey);font-size:12px;margin-top:2px;">${escapeHtml(m.code)} &middot; ${escapeHtml(m.department||'\u2014')} &middot; ${escapeHtml(m.location||'\u2014')}</div>
      </div>
    </div>`).join('');
}
function toggleMachinesDownList(){
  const panel = document.getElementById('machinesDownPanel');
  panel.style.display = panel.style.display === 'none' ? '' : 'none';
  if(panel.style.display !== 'none') panel.scrollIntoView({behavior:'smooth', block:'nearest'});
}
window.toggleMachinesDownList = toggleMachinesDownList;
...
async function populateMatrixDepartments(){
  ...
  sel.innerHTML = '<option value="">All Categories</option>' + depts.map(d=>`<option>${escapeHtml(d)}</option>`).join('');
...
async function loadMachinesPage(){
  ...
  machineState.category = 'All';
  machineState.view = localStorage.getItem('machineViewMode') || 'card';
  renderCategoryDropdown();
  applyMachineViewMode();
  renderMachines();
}
const machineState = { category: 'All', view: 'card' };

function renderCategoryDropdown(){
  const cats = [...new Set(machinesCache.map(m=>m.department).filter(Boolean))].sort();
  const counts = {};
  machinesCache.forEach(m=>{ const c = m.department || '\u2014'; counts[c] = (counts[c]||0)+1; });
  const options = ['All', ...cats];
  const sel = document.getElementById('categoryFilter');
  sel.innerHTML = options.map(c=>{
    const count = c==='All' ? machinesCache.length : (counts[c]||0);
    return `<option value="${escapeHtml(c)}" ${machineState.category===c?'selected':''}>${escapeHtml(c)} (${count})</option>`;
  }).join('');
}
function setMachineCategory(cat){
  machineState.category = cat;
  renderMachines();
}
function setMachineView(mode){
  machineState.view = mode;
  localStorage.setItem('machineViewMode', mode);
  applyMachineViewMode();
  renderMachines();
}
function applyMachineViewMode(){
  const isCard = machineState.view === 'card';
  document.getElementById('machineGrid').style.display = isCard ? '' : 'none';
  document.getElementById('machineTableWrap').style.display = isCard ? 'none' : '';
  document.getElementById('viewToggleCard').classList.toggle('active', isCard);
  document.getElementById('viewToggleTable').classList.toggle('active', !isCard);
}
function renderMachines(){
  ...
  if(machineState.category !== 'All') list = list.filter(m => (m.department||'\u2014') === machineState.category);

  const grid = document.getElementById('machineGrid');
  const tableWrap = document.getElementById('machineTableWrap');
  const empty = document.getElementById('machinesEmpty');
  if(list.length===0){
    grid.innerHTML=''; document.getElementById('machineTableBody').innerHTML='';
    empty.style.display='block'; grid.style.display='none'; tableWrap.style.display='none';
    return;
  }
  empty.style.display='none';
  applyMachineViewMode();
  ...
  if(machineState.view === 'card'){
    ...
    document.getElementById('machineTableBody').innerHTML = list.map(m=>{
      const overdue = m.nextPmDate && m.nextPmDate < today;
      const thumb = m.photoUrl
        ? `<img class="thumb" src="${m.photoUrl.replace(/"/g,'')}" alt="">`
        : `<div class="thumb-placeholder">&#128736;</div>`;
      return `
      <tr onclick="openMachineDetail('${m.id}')">
        <td>${thumb}</td>
        ...
        <td class="${overdue?'overdue':''}">${fmtDate(m.nextPmDate)}</td>
        <td>
          <div class="row-actions">
            <button class="btn-ghost small" onclick="event.stopPropagation();openMachineDetail('${m.id}')">View</button>
            ${canEdit ? `<button class="btn-primary small" onclick="event.stopPropagation();openMachineForm('${m.id}')">Edit</button>` : ''}
          </div>
        </td>
      </tr>`;
...
window.setMachineCategory = setMachineCategory;
window.setMachineView = setMachineView;
...
function openMachineForm(id){
  ...
  document.getElementById('mPhotoPreviewWrap').style.display = 'none';
  document.getElementById('mPhotoFile').value = '';
  window._removeMachinePhotoFlag = false;
  if(id){
    ...
    document.getElementById('mPhoto').value = m.legacyPhotoUrl || '';
    document.getElementById('machineFormDeleteBtn').style.display = canManageMachines() ? '' : 'none';
    if(m.hasUploadedPhoto){
      document.getElementById('mPhotoPreview').src = m.photoUrl;
      document.getElementById('mPhotoPreviewWrap').style.display = 'block';
    }
  }else{
    ...
    document.getElementById('machineFormDeleteBtn').style.display = 'none';
  }
  switchToRawView('machine-form');
}
window.openMachineForm = openMachineForm;

// Live preview when a file is chosen, before it's actually uploaded
document.getElementById('mPhotoFile').addEventListener('change', (e)=>{
  const file = e.target.files[0];
  if(!file) return;
  window._removeMachinePhotoFlag = false;
  const reader = new FileReader();
  reader.onload = (ev)=>{
    document.getElementById('mPhotoPreview').src = ev.target.result;
    document.getElementById('mPhotoPreviewWrap').style.display = 'block';
  };
  reader.readAsDataURL(file);
});

// Marks the currently-uploaded photo for removal on save (or removes it
// immediately if editing an existing machine that already has one saved).
// Doesn't touch the legacy URL field - that's independent.
async function removeMachinePhoto(){
  const id = document.getElementById('machineFormId').value;
  document.getElementById('mPhotoPreviewWrap').style.display = 'none';
  document.getElementById('mPhotoFile').value = '';
  if(id){
    try{ await apiDelete('/machines/'+id+'/photo'); showToast('Photo removed'); }
    catch(e){ showToast('Could not remove photo: '+e.message); }
  }
  window._removeMachinePhotoFlag = true;
}
...
document.getElementById('machineForm').addEventListener('submit', async (e)=>{
  ...
  try{
    let machineId = id;
    if(id){ await apiPut('/machines/'+id, payload); }
    else{ const created = await apiPost('/machines', payload); machineId = created.id; }

    const fileInput = document.getElementById('mPhotoFile');
    if(fileInput.files.length > 0){
      const fd = new FormData();
      fd.append('photo', fileInput.files[0]);
      const progressWrap = document.getElementById('mPhotoUploadProgress');
      const progressBar = document.getElementById('mPhotoUploadBar');
      const progressLabel = document.getElementById('mPhotoUploadLabel');
      progressWrap.style.display = '';
      fileInput.disabled = true;
      try{
        await new Promise((resolve, reject) => {
          const xhr = new XMLHttpRequest();
          xhr.open('POST', API+'/machines/'+machineId+'/photo');
      ...
      }catch(upErr){ showToast('Machine saved, but photo upload failed: '+upErr.message); }
      finally{ fileInput.disabled = false; progressWrap.style.display = 'none'; }
    }
...
async function openMachineDetail(id){
  ...
  try{
    const [m, metrics, findingsData] = await Promise.all([
      apiGet('/machines/'+id),
      apiGet('/machines/'+id+'/metrics').catch(()=>null),
      apiGet('/findings?machineId='+id).catch(()=>({findings:[]}))
    ]);
    const logs = m.logs || [];
    ...
    const html = `
      ...
      <div class="machine-info-grid">
        <div class="info-tile"><div class="info-tile-label">Category</div><div class="info-tile-value">${escapeHtml(m.department||'\u2014')}</div></div>
      ...
      <div class="panel-block" id="machineFindingsPanel" style="display:none;">
        <div class="panel-block-head">
          <h3>Automatic Findings</h3>
        </div>
        <div id="machineFindingsList"></div>
      </div>
    ...
    renderFindings(findingsData.findings, 'machineFindingsPanel', 'machineFindingsList');
...
function logEntryHtml(l, showMachine){
  ...
  const canReview = canManageLogs() && l.status === 'Completed';
  const reportedByBit = (l.logType==='Breakdown' && l.reportedBy) ? `${escapeHtml(l.reportedBy)}${l.priority?(' &middot; '+l.priority):''} &middot; ` : '';
  return `
    <div class="log-entry ${l.logType.toLowerCase()}">
      <div class="log-entry-top">
        <div class="log-entry-top-left">
          ...
          <span class="badge ${statusBadgeClass(l.status)}">${l.status}</span>
          <span class="log-downtime-tag">${l.downtimeHours} hr</span>
        </div>
        <span class="log-tech">${reportedByBit}${escapeHtml(l.technician)} &middot; ${fmtDateTime(l.loggedAt)}</span>
      </div>
      ...
      ${attachments.length>0 ? `<div class="attachment-list">${attachments.map(a=>`<span class="attachment-chip"><a href="javascript:void(0)" onclick="openAttachmentViewer('${a.url}','${escapeHtml(a.originalName).replace(/'/g,"\\'")}','${a.mimeType}')" title="Uploaded by ${escapeHtml(a.uploadedBy||'unknown')} on ${fmtDateTime(a.uploadedAt)}">&#128206; ${escapeHtml(a.originalName)}</a>${canManageLogs() ? `<button class="del" onclick="deleteAttachment('${a.id}','${l.id}')" title="Delete attachment">&times;</button>` : ''}</span>`).join('')}</div>` : ''}
      <div class="log-entry-actions">
        ...
        ${canManageLogs() ? `<button class="log-attach-btn" onclick="editLogForm('${l.id}')">Edit</button>` : ''}
...
let attachUploadInProgress = false;
function triggerAttachUpload(logId){
  if(attachUploadInProgress){ showToast('An upload is already in progress - please wait for it to finish'); return; }
  ...
  input.onchange = async () => {
    ...
    attachUploadInProgress = true;
    try{
      await new Promise((resolve, reject) => {
        const xhr = new XMLHttpRequest();
        xhr.open('POST', API+'/logs/'+logId+'/attachments');
        xhr.withCredentials = true;
        xhr.upload.onprogress = (evt) => {
          if(evt.lengthComputable){
            const pct = Math.round((evt.loaded / evt.total) * 100);
            showToast(`Uploading\u2026 ${pct}%`, true);
          }
        };
        xhr.onload = () => {
          if(xhr.status >= 200 && xhr.status < 300) resolve();
          else { let msg='Upload failed'; try{ msg = JSON.parse(xhr.responseText).error || msg; }catch(_){} reject(new Error(msg)); }
        };
        xhr.onerror = () => reject(new Error('Network error during upload'));
        xhr.send(fd);
      });
    ...
    finally{ attachUploadInProgress = false; }
  };
  input.click();
}
...
async function deleteLog(id, machineCode){
  confirmWithPassword(
    'Delete this log?',
    `This removes the log for ${machineCode} and any attached files permanently - this cannot be undone.`,
    async (password) => {
      await apiDelete('/logs/'+id, { password });
...
async function deleteMachine(id, name){
  confirmWithPassword(
    'Delete this machine?',
    `This permanently removes "${name}" from your registry. If it has any maintenance logs attached, this will be blocked - delete those first if you're sure.`,
    async (password) => {
      await apiDelete('/machines/'+id, { password });
...
async function loadLogsPage(){
  ...
  // Default the visible list to the current month rather than showing every
  // log ever recorded - previously there was no date filter on this list at
  // all, only on the Excel export, so this page could require a lot of
  // scrolling once enough history built up.
  const fromEl = document.getElementById('logFilterFrom');
  const toEl = document.getElementById('logFilterTo');
  if(!fromEl.value && !toEl.value){
    const now = new Date();
    fromEl.value = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-01`;
    toEl.value = todayISO();
  }
  logViewState.view = localStorage.getItem('logViewMode') || 'card';
  applyLogViewMode();
  renderLogsPage();
}

function clearLogFilters(){
  document.getElementById('logMachineFilter').value = '';
...
async function exportLogsExcel(){
  // Uses the same From/To dates already set in the main filter bar above,
  // rather than asking for a separate date range just for the download -
  // one filter drives both what you see and what you export.
  const from = document.getElementById('logFilterFrom').value;
  const to = document.getElementById('logFilterTo').value;
  if(!from || !to){ showToast('Set a From and To date in the filters above first'); return; }
  const url = '/logs/export/excel?from=' + from + '&to=' + to;
  try{
...
const logViewState = { view: 'card' };
function setLogView(mode){
  logViewState.view = mode;
  localStorage.setItem('logViewMode', mode);
  applyLogViewMode();
  renderLogsPage();
}
function applyLogViewMode(){
  const isCard = logViewState.view === 'card';
  document.getElementById('logsPageList').style.display = isCard ? '' : 'none';
  document.getElementById('logsTableWrap').style.display = isCard ? 'none' : '';
  document.getElementById('logViewToggleCard').classList.toggle('active', isCard);
  document.getElementById('logViewToggleTable').classList.toggle('active', !isCard);
}
...
async function renderLogsPage(){
  ...
  try{
    ...
    tableBody.innerHTML = logs.map(l=>logTableRowHtml(l)).join('');
...
function logTableRowHtml(l){
  logCache[l.id] = l;
  const findingsFull = escapeHtml(l.findings||'');
  const actionsFull = escapeHtml(l.actionsTaken||'');
  const machineFull = escapeHtml(`${l.machineName} (${l.machineCode})`);
  const startDisp = l.startTime ? fmtTimeOnly(l.startTime) : '\u2014';
  const endDisp = l.endTime ? fmtTimeOnly(l.endTime) : '\u2014';
  return `
    <tr>
      <td class="nowrap">${fmtDate(l.loggedAt.slice(0,10))}</td>
      <td class="crop-cell" title="${machineFull}">${machineFull}</td>
      <td class="nowrap">${startDisp}</td>
      <td class="nowrap">${endDisp}</td>
      <td class="nowrap">${l.downtimeHours}</td>
      <td class="wrap-cell-wide" title="${findingsFull}">${findingsFull}</td>
      <td class="wrap-cell-wide" title="${actionsFull}">${actionsFull}</td>
      <td class="crop-cell" title="${escapeHtml(l.partsUsed||'')}">${escapeHtml(l.partsUsed||'\u2014')}</td>
      <td class="nowrap">${escapeHtml(l.technician)}</td>
      <td class="nowrap"><span class="badge ${statusBadgeClass(l.status)}">${l.status}</span></td>
      <td class="nowrap">
        <div class="icon-btn-row">
          ${canManageLogs() ? `<button class="icon-btn" title="Edit" onclick="editLogForm('${l.id}')">&#9998;</button>` : ''}
          <button class="icon-btn" title="Download PDF" onclick="downloadLogPDF('${l.id}')">&#11015;</button>
          <button class="icon-btn" title="Attach File" onclick="triggerAttachUpload('${l.id}')">&#128206;</button>
        </div>
      </td>
    </tr>`;
}
...
// ============ MACHINE PICKER (type-to-filter, replaces a 200+ option select) ============
// The hidden #logMachine input is the actual source of truth submitted with
// the form. It's cleared on every keystroke so a stale selection can never
// be submitted alongside text that no longer matches it - the user must
// click an actual result to set it again.
function filterMachinePicker(query){
  const resultsBox = document.getElementById('logMachineResults');
  document.getElementById('logMachine').value = '';
  const q = (query || '').trim().toLowerCase();
  if(!q){ resultsBox.style.display = 'none'; resultsBox.innerHTML = ''; return; }
  const matches = machinesCache.filter(m =>
    m.name.toLowerCase().includes(q) || m.code.toLowerCase().includes(q)
  ).slice(0, 5);
  if(matches.length === 0){
    resultsBox.innerHTML = '<div class="machine-picker-empty">No matching machines</div>';
  }else{
    resultsBox.innerHTML = matches.map(m =>
      `<div class="machine-picker-item" onclick="selectMachineFromPicker('${m.id}')">${escapeHtml(m.name)} <span class="machine-picker-code">(${escapeHtml(m.code)})</span></div>`
    ).join('');
  }
  resultsBox.style.display = 'block';
}
document.getElementById('logMachineSearch').addEventListener('input', (e)=> filterMachinePicker(e.target.value));
document.addEventListener('click', (e)=>{
  const picker = document.getElementById('logMachineSearch');
  const results = document.getElementById('logMachineResults');
...
function selectMachineFromPicker(machineId){
  const m = machinesCache.find(x=>x.id===machineId);
  if(!m) return;
  document.getElementById('logMachineSearch').value = `${m.name} (${m.code})`;
  document.getElementById('logMachine').value = m.id;
  document.getElementById('logMachineResults').style.display = 'none';
}
window.selectMachineFromPicker = selectMachineFromPicker;

// Sets the picker to a specific machine programmatically (used by
// openLogForm/editLogForm), or clears it entirely if no id is given.
function setMachinePicker(machineId){
  const searchBox = document.getElementById('logMachineSearch');
  const hidden = document.getElementById('logMachine');
  if(machineId){
    const m = machinesCache.find(x=>x.id===machineId);
    if(m){
      searchBox.value = `${m.name} (${m.code})`;
      hidden.value = m.id;
      return;
    }
  }
...
async function openLogForm(preselectMachineId){
  editingLogId = null;
  document.getElementById('logFormDeleteBtn').style.display = 'none';
  await loadMachinesCache();
  document.getElementById('logForm').reset();
  document.getElementById('logMachineSearch').disabled = machinesCache.length === 0;
  document.getElementById('logMachineSearch').placeholder = machinesCache.length === 0
    ? 'No machines registered — add one first'
    : 'Type a machine name or code...';
  setMachinePicker(preselectMachineId || null);
  ...
  document.getElementById('logStartDate').value = nowLocalDateStr();
  document.getElementById('logStartTime').value = nowLocalTimeStr();
  document.getElementById('logEndDate').value = nowLocalDateStr();
  delete document.getElementById('logEndDate').dataset.manuallySet;
  document.getElementById('logEndTime').value = nowLocalTimeStr();
  document.getElementById('logTimeError').style.display = 'none';
  updateLogDowntimeDisplay();
  switchToRawView('log-form');
}
window.openLogForm = openLogForm;

// Supervisor/Management/HOD only - correcting an existing log's details.
// Reuses the same form as creation; attachments are managed separately
// per-log (not through this form). Older logs created before Start/End
// times existed fall back to the same derived-estimate calculation the
// PDF/Excel exports used to use everywhere (End = logged_at, Start = End
// minus downtime hours) - editing and resaving such a log switches it over
// to real Start/End times going forward.
async function editLogForm(logId){
  if(!canManageLogs()){ showToast('Only supervisors and management can edit logs'); return; }
  const l = logCache[logId];
  if(!l){ showToast('Log details not available to edit'); return; }
  editingLogId = logId;
  await loadMachinesCache();
  setMachinePicker(l.machineId);
  ...
  if(l.startTime && l.endTime){
    startIso = l.startTime; endIso = l.endTime;
  }else{
    endIso = l.loggedAt || new Date().toISOString();
    startIso = new Date(new Date(endIso).getTime() - (l.downtimeHours || 0) * 3600000).toISOString();
  }
  const startParts = isoToLocalDateTimeParts(startIso);
  const endParts = isoToLocalDateTimeParts(endIso);
  document.getElementById('logStartDate').value = startParts.date;
  document.getElementById('logStartTime').value = startParts.time;
  document.getElementById('logEndDate').value = endParts.date;
  document.getElementById('logEndTime').value = endParts.time;
  if(endParts.date !== startParts.date) document.getElementById('logEndDate').dataset.manuallySet = '1';
  else delete document.getElementById('logEndDate').dataset.manuallySet;
  document.getElementById('logTimeError').style.display = 'none';
  updateLogDowntimeDisplay();
  ...
  document.getElementById('logFormDeleteBtn').style.display = canManageLogs() ? '' : 'none';
  switchToRawView('log-form');
}
...
document.getElementById('logForm').addEventListener('submit', async (e)=>{
  ...
  if(!machineId){ showToast('Type a machine name and pick it from the results list'); return; }

  const startISO = localDateTimeToISO(document.getElementById('logStartDate').value, document.getElementById('logStartTime').value);
  const endISO = localDateTimeToISO(effectiveEndDate(), document.getElementById('logEndTime').value);
  if(!startISO || !endISO){ showToast('Fill in both Start and End date/time'); return; }
...
['logStartDate','logStartTime','logEndDate','logEndTime'].forEach(id=>{
  document.getElementById(id).addEventListener('change', updateLogDowntimeDisplay);
});

// End Date auto-follows Start Date for the common same-day case, so most
// logs never need it touched at all. Once a user has manually set End Date
// to something different from Start Date (a real multi-day case), this
// stops overwriting it - tracked via a data attribute rather than just
// comparing values, so deliberately setting them equal for a genuine
// same-day multi-shift log doesn't accidentally re-enable auto-follow.
document.getElementById('logStartDate').addEventListener('input', (e)=>{
  const endDateField = document.getElementById('logEndDate');
  if(endDateField.dataset.manuallySet !== '1'){
    endDateField.value = e.target.value;
  }
});
document.getElementById('logEndDate').addEventListener('input', ()=>{
  document.getElementById('logEndDate').dataset.manuallySet = '1';
});
...
async function loadCustomRolesCache(){
  try{ customRolesCache = await apiGet('/custom-roles'); }catch(e){ customRolesCache = []; }
}

// The uRole select mixes two kinds of options: the 4 static built-in roles
// (value = the role name itself) and dynamically-appended custom roles
// (value = "custom:<id>", since a custom role's name alone isn't a safe
// select value - two different orgs, or even the same org over time,
// could otherwise collide). This reads whichever was picked and returns
// the right shape for the API.
...
function populateRoleSelect(selectedValue){
  ...
  customRolesCache.forEach(r=>{
    const opt = document.createElement('option');
    opt.value = 'custom:'+r.id;
    opt.textContent = r.name+' (Custom)';
...
const PERMISSION_LABELS = {
  view_dashboard: 'View Dashboard', view_machines: 'View Machines', manage_machines: 'Manage Machines',
  submit_logs: 'Submit Logs', manage_logs: 'Manage Logs', view_reports: 'View Reports',
  manage_users: 'Manage Users', manage_branding: 'Manage Branding'
};

async function openCustomRoles(){
  if(!isManagement()){ showToast('Only management can manage custom roles'); return; }
  switchToRawView('custom-roles');
  document.getElementById('customRoleFormPanel').style.display = 'none';
  await renderCustomRolesList();
}
window.openCustomRoles = openCustomRoles;

async function renderCustomRolesList(){
  try{
    customRolesCache = await apiGet('/custom-roles');
    const list = document.getElementById('customRolesList');
    const empty = document.getElementById('customRolesEmpty');
    if(customRolesCache.length===0){ list.innerHTML=''; empty.style.display='block'; return; }
    empty.style.display='none';
    list.innerHTML = customRolesCache.map(r=>`
      <div class="user-row">
        <div class="user-row-left">
          <span class="user-name">${escapeHtml(r.name)}</span>
          <span class="user-meta">${r.permissions.map(p=>escapeHtml(PERMISSION_LABELS[p]||p)).join(', ')}</span>
        </div>
        <div class="user-row-actions">
          <button class="btn-ghost" style="padding:7px 12px;font-size:12px;" onclick="openCustomRoleForm('${r.id}')">Edit</button>
          <button class="btn-danger" onclick="deleteCustomRole('${r.id}','${escapeHtml(r.name)}')">Delete</button>
        </div>
      </div>`).join('');
  }catch(e){ showToast('Could not load custom roles: '+e.message); }
}

function openCustomRoleForm(id){
  document.getElementById('customRoleForm').reset();
  document.getElementById('customRoleError').style.display = 'none';
  document.getElementById('customRoleId').value = id || '';
  document.querySelectorAll('#customRoleForm .permission-check input').forEach(cb=>cb.checked = false);
  if(id){
    const r = customRolesCache.find(x=>x.id===id);
    if(!r){ showToast('Role not found'); return; }
    document.getElementById('customRoleFormTitle').textContent = 'Edit Custom Role';
    document.getElementById('customRoleFormSubmit').textContent = 'Save Changes';
    document.getElementById('customRoleName').value = r.name;
    r.permissions.forEach(p=>{
      const cb = document.querySelector(`#customRoleForm .permission-check input[value="${p}"]`);
      if(cb) cb.checked = true;
    });
  }else{
    document.getElementById('customRoleFormTitle').textContent = 'New Custom Role';
    document.getElementById('customRoleFormSubmit').textContent = 'Create Role';
  }
  document.getElementById('customRoleFormPanel').style.display = 'block';
}
window.openCustomRoleForm = openCustomRoleForm;

function closeCustomRoleForm(){
  document.getElementById('customRoleFormPanel').style.display = 'none';
}
window.closeCustomRoleForm = closeCustomRoleForm;

document.getElementById('customRoleForm').addEventListener('submit', async (e)=>{
  e.preventDefault();
  const errEl = document.getElementById('customRoleError');
  errEl.style.display = 'none';
  const id = document.getElementById('customRoleId').value;
  const name = document.getElementById('customRoleName').value.trim();
  const permissions = Array.from(document.querySelectorAll('#customRoleForm .permission-check input:checked')).map(cb=>cb.value);
  if(permissions.length===0){ errEl.textContent = 'Select at least one permission'; errEl.style.display='block'; return; }
  const submitBtn = e.target.querySelector('button[type="submit"]');
  submitBtn.disabled = true;
  try{
    if(id) await apiPut('/custom-roles/'+id, { name, permissions });
    else await apiPost('/custom-roles', { name, permissions });
...
async function deleteCustomRole(id, name){
  if(!confirm(`Delete custom role "${name}"? Users with no other role assigned would be affected - reassign them first if needed.`)) return;
  try{
    await apiDelete('/custom-roles/'+id);
    showToast('Role deleted');
    await renderCustomRolesList();
  }catch(e){ showToast('Could not delete role: '+e.message); }
}
...
async function openUserForm(id){
  ...
  if(id){
    ...
    populateRoleSelect(u.customRoleId ? ('custom:'+u.customRoleId) : u.role);
...
document.getElementById('userForm').addEventListener('submit', async (e)=>{
  ...
  try{
    if(id){
      const payload = { name: document.getElementById('uName').value.trim(), ...readSelectedRole() };
...
function renderDelta(elId, data){
  const el = document.getElementById(elId);
  if(!el || !data){ if(el) el.innerHTML=''; return; }
  // Only render whichever comparisons are actually present - Daily only
  // has vsYesterday, Monthly has vsLastMonth + vsLastYear, Yearly only
  // has vsLastYear. Each report shows just what makes sense at its own
  // time granularity, not every possible comparison crammed together.
  const possible = [
    ['yesterday', data.vsYesterday],
    ['last month', data.vsLastMonth],
    ['last year', data.vsLastYear]
  ];
  const rows = possible.filter(([label, d]) => d);
  el.innerHTML = rows.map(([label, d])=>{
    const arrow = d.direction==='up' ? '&#9650;' : d.direction==='down' ? '&#9660;' : '&#8212;';
    const pctText = d.pct===null ? '' : ` ${d.pct}%`;
    return `<div class="d-row"><span class="${d.direction}">${arrow}${pctText}</span> vs ${label} (${d.refVal}&rarr;${data.current})</div>`;
  }).join('');
}
...
async function loadDailyReport(){
  ...
  try{
    ...
    if(data.comparison){
      renderDelta('dailyBreakdownDelta', data.comparison.breakdownCount);
      renderDelta('dailyDowntimeDelta', data.comparison.totalDowntime);
    }
    destroyChart('daily');
...
// Shared by Monthly/Yearly reports - Parts Used is free text, not
// structured data, so this lists actual entries (what/where/when) rather
// than attempting a grouped count that could misrepresent what the
// technician actually wrote.
function renderPartsUsedList(containerId, summary){
  const el = document.getElementById(containerId);
  if(!summary || summary.length===0){
    el.innerHTML = '<div class="empty-state"><div class="big">No parts logged in this period</div></div>';
    return;
  }
  el.innerHTML = summary.map(p=>`
    <div class="recent-log-row ${p.logId?'clickable':''}" ${p.logId?`onclick="openLogDetail('${p.logId}')"`:''}>
      <div>
        <b>${escapeHtml(p.partsUsed)}</b>
        <div style="color:var(--grey);font-size:12px;margin-top:2px;">${escapeHtml(p.machineName)} (${escapeHtml(p.machineCode)}) &middot; ${fmtDate(p.date)}</div>
      </div>
    </div>`).join('');
}

async function openLogDetail(logId){
  window._logDetailBackView = 'reports';
  switchToRawView('log-detail');
  document.getElementById('logDetailContent').innerHTML = '<div class="empty-state">Loading&hellip;</div>';
  try{
    const log = await apiGet('/logs/'+logId);
    document.getElementById('logDetailContent').innerHTML = `
      <h2 style="font-family:'Oswald',sans-serif;text-transform:uppercase;font-size:17px;margin-bottom:14px;color:var(--navy);">Maintenance Log Details</h2>
      ${logEntryHtml(log, true)}
    `;
  }catch(e){
    document.getElementById('logDetailContent').innerHTML = `<div class="empty-state"><div class="big">Could not load log</div><div>${escapeHtml(e.message)}</div></div>`;
  }
}
...
async function loadMonthlyReport(){
  ...
  try{
    ...
    if(data.comparison){
      renderDelta('monthlyBreakdownDelta', data.comparison.breakdownCount);
      renderDelta('monthlyDowntimeDelta', data.comparison.totalDowntime);
    }
    renderPartsUsedList('monthlyPartsList', data.partsUsedSummary);
...
async function loadYearlyReport(){
  ...
  try{
    ...
    if(data.comparison){
      renderDelta('yearlyBreakdownDelta', data.comparison.breakdownCount);
      renderDelta('yearlyDowntimeDelta', data.comparison.totalDowntime);
    }
    renderPartsUsedList('yearlyPartsList', data.partsUsedSummary);
...
async function downloadLogPDF(id){
  ...
  // breakdowns); falls back to the old derived estimate + disclaimer only
  // for logs created before Start/End times existed.
  ...
  field('Date', spansMultipleDays ? `${dateFmt(startDate)} \u2013 ${dateFmt(endDate)}` : fmtDate(l.loggedAt.slice(0,10)));
  ...
  field('Category', l.machineDepartment || 'N/A');
  y += 2;

  sectionHeading('3. MAINTENANCE DETAILS');
  const startLabel = spansMultipleDays ? `${dateFmt(startDate)}, ${timeFmt(startDate)}` : timeFmt(startDate);
  const endLabel = spansMultipleDays ? `${dateFmt(endDate)}, ${timeFmt(endDate)}` : timeFmt(endDate);
  field(hasRealTimes ? 'Start Time' : 'Start Time (est.)', startLabel);
  field(hasRealTimes ? 'End Time' : 'End Time (est.)', endLabel);
