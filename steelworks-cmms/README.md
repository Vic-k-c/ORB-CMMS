...
**Favicon (browser tab icon):** generated automatically from the
uploaded logo on upload (using `jimp`, a pure-JS image library — no
native compilation, safe for Render). Wide "wordmark" logos (company
name spelled out, wider than roughly 1.8:1) get cropped to a square from
the **left edge** first — that's usually where a leading letter or icon
mark sits — rather than squishing the whole wordmark into an illegibly
tiny square. Logos that are already close to square are used as-is. The
favicon updates on the login page too, for whichever organization is
currently remembered on that device.

**Logo sizing is aspect-ratio-aware**, not a fixed square, everywhere
the logo appears — the app header (responsive width, roughly 45vw
capped at 220px, so a wide wordmark logo gets real room while a square
icon-style logo stays compact instead of being stretched to fill space
it doesn't need), the PDF letterheads, and both Excel exports. A fixed
square box used to either squish wide logos or leave small square ones
looking tiny — this reads the logo's real uploaded width/height
(`organizations.logo_width`/`logo_height`) and sizes proportionally, at
a fixed target height with a width cap generous enough that even a very
wide "letterhead-style" logo (spelled-out name + tagline baked into one
wide image, 5:1 or more) still renders at its full intended height
rather than being squeezed short to fit the cap:

- Individual log PDF: 16mm target height, capped at 88mm wide (leaves
  room before the document-control box divider)
- Daily/Monthly/Yearly report PDFs: 16mm target height, capped at
  100mm wide (no document-control box competing for space there)
- Both Excel exports (shared logic): 60px target height, capped at
  230px wide — deliberately the tighter of the two, since it's capped
  by the machine-performance sheet's merged brand cell area (~252px),
  the smaller of the two sheets this logic serves. A logo wide enough
  to hit this cap won't reach its full 60px height the way it can in
  the PDFs, which have a whole page width to work with — fully solving
  that would mean widening the merged cell range itself and shifting
  the document-control columns over, a bigger structural change than
  a sizing formula can fix on its own.

**"CMMS" identity badge:** once an organization's own logo has replaced
the default "STEELWORKS CMMS" branding, nothing else indicates what kind
of software this is — so a small, fixed "CMMS" badge appears near the
logo in the header (and a one-line "Computerized Maintenance Management
System" tag on the login page), independent of the three display modes
above. This is deliberate: it identifies the *platform*, not the
organization's brand, so even "Logo only" mode stays true to its name
for org-specific branding while a first-time user can still tell what
they're looking at.
...
technician, Start/End date and time (downtime hours recalculates
automatically from these), findings, actions taken, parts, and status
(Pending/Completed). Technicians cannot edit logs, including their own.
...
**Start and End date/time are real technician-entered fields** in the
log form, not calculated after the fact — this supports breakdowns that
span more than one calendar day (e.g. started Monday evening, resolved
Wednesday morning). **Downtime Hours is always derived from Start/End**,
shown as a read-only, live-recalculating field in the form rather than
typed in separately, so the two numbers can never disagree. When a log's
Start and End fall on different days, the PDF and Excel export both show
the date alongside the time for each (e.g. "03 Sep 2026, 22:06") instead
of just a bare clock time, and the PDF's Date field shows a range.

**Logs created before this existed** don't have real Start/End values
stored — for those, the PDF and Excel export fall back to the original
calculated approach (End = when the log was submitted, Start = End minus
downtime hours) and the PDF marks both fields "(est.)" with a note
explaining they're estimated, not technician-entered. Editing and
resaving one of these older logs switches it over to real Start/End
...
| GET    | `/api/public/org-favicon/:orgCode`   | Public                  | Favicon image only, same purpose as above |
...
| PUT    | `/api/organization/branding`         | Management, Maintenance HOD           | Update own organization's branding fields |
| GET    | `/api/organization/logo`             | Any org user             | Stream own organization's logo file |
| GET    | `/api/organization/favicon`          | Any org user             | Stream own organization's generated favicon |
| POST   | `/api/organization/logo`             | Management, Maintenance HOD           | Upload/replace own organization's logo |
| DELETE | `/api/organization/logo`             | Management, Maintenance HOD           | Remove own organization's logo |
...
| POST   | `/api/machines`                      | Supervisor, Management, Maintenance HOD             | Register a machine |
| PUT    | `/api/machines/:id`                  | Supervisor, Management, Maintenance HOD             | Update a machine |
| GET    | `/api/machines/:id/metrics`          | Any org user              | Lifetime reliability metrics for one machine |
| GET    | `/api/machines/metrics-matrix`       | Supervisor, Management, Maintenance HOD             | All machines' metrics for a period |
| GET    | `/api/machines/metrics-matrix/export/excel` | Supervisor, Management, Maintenance HOD             | Excel export of the performance matrix |
| GET    | `/api/logs`                          | Any org user             | List logs, `?machineId=&logType=&status=` |
| POST   | `/api/logs`                          | Any org user             | Submit a maintenance log (startTime/endTime required; downtime auto-computed) |
| PUT    | `/api/logs/:id`                      | Supervisor, Management, Maintenance HOD | Edit an existing log's content, including Start/End time |
| PATCH  | `/api/logs/:id/review`               | Supervisor, Management, Maintenance HOD             | Mark a Completed log as Reviewed |
...
| DELETE | `/api/attachments/:id`               | Supervisor, Management, Maintenance HOD             | Delete an attachment |
...
| GET    | `/api/users`                         | Management, Maintenance HOD           | List own organization's user accounts |
| POST   | `/api/users`                         | Management, Maintenance HOD           | Create a user account (own org) |
| PUT    | `/api/users/:id`                     | Management, Maintenance HOD           | Update name/role/password (own org) |
| DELETE | `/api/users/:id`                     | Management, Maintenance HOD           | Delete a user account (own org) |
| GET    | `/api/settings`                      | Supervisor, Management, Maintenance HOD             | Own organization's default operating hours/day |
| PUT    | `/api/settings`                      | Supervisor, Management, Maintenance HOD             | Update default operating hours/day |
| GET    | `/api/schedule-overrides`            | Supervisor, Management, Maintenance HOD             | List date overrides, optional `?from=&to=` |
| POST   | `/api/schedule-overrides`             | Supervisor, Management, Maintenance HOD             | Add/update an override for one date |
| DELETE | `/api/schedule-overrides/:date`      | Supervisor, Management, Maintenance HOD             | Remove an override |
