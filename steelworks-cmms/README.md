...
Maintenance Supervisor and Management can click **Edit** on any log card to
correct its details after submission — machine, type, technician, downtime,
findings, actions taken, parts, and status (Pending/Completed). Technicians
cannot edit logs, including their own.

Two things are deliberately **not** editable through this form:
- **The log's timestamp** — precise date/time correction is rare enough
  that a direct database fix is safer than adding timezone-conversion
  logic to a form. Ask if this comes up often enough to be worth building.
...
- **Dashboard → Machine Performance** (Maintenance Supervisor and
  Management only) — the same five metrics for every machine in that
  organization side by side, filterable by Daily / Monthly / Yearly /
  Custom Range, with its own Excel export.
...
| Action                        | Technician | Maintenance Supervisor | Management | Super Admin |
|--------------------------------|:----------:|:-----------------------:|:----------:|:-----------:|
| View dashboard, machines, logs, reports (own org) | Yes | Yes | Yes | No |
| Submit a maintenance log       | Yes        | Yes                     | Yes        | No |
| Attach files to a log          | Yes        | Yes                     | Yes        | No |
| Download PDF / Excel reports   | Yes        | Yes                     | Yes        | No |
| Mark a log as Reviewed         | No         | Yes                     | Yes        | No |
| Add / edit machines            | No         | Yes                     | Yes        | No |
| Delete an attachment           | No         | Yes                     | Yes        | No |
| View Dashboard performance matrix | No     | Yes                     | Yes        | No |
| Edit operating schedule        | No         | Yes                     | Yes        | No |
| Manage user accounts (own org) | No         | No                      | Yes        | No |
| Edit organization branding     | No         | No                      | Yes        | No |
| Create new organizations       | No         | No                      | No         | Yes |
...
**Render's free-tier web service has no persistent local disk** — even
though the database itself (Postgres) now persists your machines/logs/
users independently of the web service, uploaded **attachments and
logos still live on the web service's local disk** and will be wiped on
redeploy or after ~15 minutes idle. If file uploads need to survive
long-term on Render, they'd need to move to an external object store
(e.g. S3-compatible storage) — not implemented here yet.
...
| PUT    | `/api/logs/:id`                      | Supervisor, Management   | Edit an existing log's content (not its timestamp) |
